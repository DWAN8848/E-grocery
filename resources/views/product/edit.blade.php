...
@section('content')
    ...
    <form action="{{route('product.update',$product->id)}}" method="POST" class="mt-5" enctype="multipart/form-data">
        ...
        <p>Current Image</p>
        <img class="w-14" src="{{asset('images/products/'.$product->photopath)}}" alt="">
        ...
        <div class="flex">
            <input type="submit" class="bg-blue-600 text-white px-4 py-2 mx-2 rounded-lg cursor-pointer" value="Update Product">

            <a href="{{route('product.index')}}" class="bg-red-600 text-white px-10 py-2 mx-2 rounded-lg cursor-pointer">Exit</a>
        </div>
    </form>
